      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ...
            ['-q, --quiet', 'Suppress non-essential logs and fast-path banners; command results still print'],
            ['--json', 'Request JSON where that command implements it; do not assume one stable schema across commands'],
        ...
        },
        {
          ...
          rows: [
            ...
            ['audit', '`sbom --output`; `secrets --path`; `log --limit --severity --export`; `slsa <package> [--certificate-identity]`; `licenses --format --export --filter --check-policy`; `fix --dry-run --yes --min-severity`; `export --framework --period --output`'],
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['omg clean', 'Remove orphans or caches; --orphans, --cache, --aur, --all, --dry-run, --yes'],
            ['omg explicit', 'List explicitly installed packages; --count prints only the count'],
            ['omg ec / tc / oc / uc', 'Fast explicit, total, orphan, and available-update counters'],
    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ['node', 'Alias nodejs; .node-version, .nvmrc, package.json, and .tool-versions; node, npm, npx'],
            ['python', 'Alias python3; .python-version, pyproject.toml, and .tool-versions; python3, pip'],
            ['go', 'Alias golang; .go-version, go.mod, and .tool-versions; go, gofmt'],
            ['rust', 'Alias rustlang; rust-toolchain, rust-toolchain.toml, and .tool-versions; rustc, cargo'],
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['omg hooks install', 'Install environment-sync Git hooks; --force overwrites existing hooks'],
      ...
      blocks: [
        {
          ...
          columns: ['Command', 'Purpose and options'],
          rows: [
            ['omg status', 'Show package totals, updates, orphans, vulnerabilities, and cached runtimes; --fast uses counts only'],
            ['omg doctor', 'Check network, backend, daemon, PATH, and shell hook; --network tests mirrors, --eol checks runtime EOL, --turbo primes sudo'],
            ['omg audit [scan]', 'Scan installed packages for advisories'],
            ['omg audit sbom', 'Write an Arch package CycloneDX inventory; -o/--output selects the file'],
            ['omg audit secrets', 'Scan a directory for credentials; -p/--path selects it'],
            ['omg audit log', 'Read or export audit entries; --limit, --severity, and --export filter output'],
            ['omg audit verify / policy', 'Check local audit-chain consistency or show policy status'],
            ['omg audit slsa <package>', 'Verify supported artifact signatures; optional --certificate-identity binds the Fulcio SAN'],
            ['omg audit licenses', 'Report license inventory; --format, --export, --filter, and --check-policy'],
            ['omg audit fix', 'Upgrade vulnerable packages; --dry-run, --yes, and --min-severity'],
            ['omg audit export', 'Export generic compliance evidence; --framework, --period, and --output'],
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['omg init', 'Run first setup; --defaults, --skip-shell, and --skip-daemon make it non-interactive'],
          ],
        },
        {
          kind: 'table',
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['omg env share', 'Publish a GitHub Gist; --description and --public control metadata and visibility'],
            ...
            ['omg snapshot create / list', 'Create a state snapshot with optional --message or list snapshots'],
            ['omg snapshot restore <id>', 'Preview or restore a snapshot with --dry-run and --yes'],
            ['omg snapshot delete <id>', 'Delete one snapshot'],
            ['omg migrate export', 'Write a portable manifest; -o/--output defaults to omg-manifest.json'],
        ...
        },
        {
          ...
          columns: ['Command', 'Purpose and options'],
          rows: [
            ['omg history', 'Show transactions; --limit, --search, --type, --from, and --to filter entries'],
      ...
      blocks: [
        {
          ...
          rows: [
            ...
            ['omg team compliance', 'Check compliance; --export writes a report and --enforce blocks non-compliant operations'],
        ...
        },
        {
          ...
          rows: [
            ...
            ['omg container shell', 'Open a project-mounted shell; --image, --workdir, --env, and --volume'],
            ['omg container build', 'Build an image; --dockerfile, --tag, --no-cache, --build-arg, and --target'],
            ...
            ['omg account link / status / unlink', 'Optional dashboard link; --token-stdin avoids putting the token in shell history'],
          ],
        },
      ],
    },
      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ...
            ['omg enterprise reports', 'Create monthly, quarterly, or custom executive reports with --report-type'],
            ['omg enterprise policy show', 'Show policies, optionally scoped with --scope'],
            ['omg enterprise audit-export', 'Export generic evidence with --framework, --period, and --output'],
            ['omg enterprise license-scan', 'Scan license compliance; --export accepts json or csv'],
          ],
        },
        {
          kind: 'note',
